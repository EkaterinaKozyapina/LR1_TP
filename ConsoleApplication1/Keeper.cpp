...
void Keeper::extractFromFile() {
	...
	if (fin.is_open())
	{
		...
		for (int i = 0; i < intBuf; i++)
		{
			...
			if (strBuf == "Поэт")
			{
				pubBuf = new Poet(fio, stoi(date1), stoi(date2), vecBuf, nick);
			}
			else if (strBuf == "Романист")
			{
				pubBuf = new Novelist(fio, stoi(date1), stoi(date2), vecBuf, bio);
			}
			else if (strBuf == "Фантаст")
			{
				pubBuf = new Fantasist(fio, stoi(date1), stoi(date2), vecBuf, exs);
			}
